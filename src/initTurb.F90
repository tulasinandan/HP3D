...
!   THIS IS FOR 2D INITIAL CONDITIONS THAT I GET FROM BILL'S CODE 
    ...
    read(8,rec=1) tmp
    close(8)
    np(1:nx*pex,1:ny*pey,1)=tmp
    np(0,:,1)=np(nx*pex,:,1) ; np(nx*pex+1,:,1)=np(1,:,1)
    np(:,0,1)=np(:,ny*pey,1) ; np(:,ny*pey+1,1)=np(:,1,1)

!   fbx=fbx*sqrt(b0x**2+b0y**2+b0z**2)/5.; 
!   fby=fby*sqrt(b0x**2+b0y**2+b0z**2)/5.; 
!   fbz=fbz*sqrt(b0x**2+b0y**2+b0z**2)/5.
!   fvx=fvx*sqrt(b0x**2+b0y**2+b0z**2)/5.; 
!   fvy=fvy*sqrt(b0x**2+b0y**2+b0z**2)/5.; 
!   fvz=fvz*sqrt(b0x**2+b0y**2+b0z**2)/5.
! 
!   fbz=0.; fvz=0.
!
!   fbx=fbx/8.; fby=fby/8.; fbz=fbz/8.
!   fvx=fvx/8.; fvy=fvy/8.; fvz=fvz/8.
