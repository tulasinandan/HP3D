...
!                                                    fvz
  ...
  Real(kind=drk), Dimension(1:nx*pex,1:ny*pey,1:nz*pez) :: tmp
  integer :: ixx, iyy, izz, nnnn
...
!   THIS IS TO READ 3D INITIAL CONDITIONS
    ixx=my_pex*nx; iyy=my_pey*ny; izz=my_pez*nz
!   write(6,*) 'myproc, ixx, iyy',my_pex,my_pey,ixx,iyy
!   print*, 'my proc, izz+1, izz+nz',my_pex,my_pey,my_pez,izz+1,izz+nz

    open(unit=8,status='old',file='Fbx.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    fbx(1:nx,1:ny,1:nz) = tmp(ixx+1:ixx+nx,iyy+1:iyy+ny,izz+1:izz+nz)

    open(unit=8,status='old',file='Fby.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    fby(1:nx,1:ny,1:nz) = tmp(ixx+1:ixx+nx,iyy+1:iyy+ny,izz+1:izz+nz)

    open(unit=8,status='old',file='Fbz.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    fbz(1:nx,1:ny,1:nz) = tmp(ixx+1:ixx+nx,iyy+1:iyy+ny,izz+1:izz+nz)

    open(unit=8,status='old',file='Fvx.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    fvx(1:nx,1:ny,1:nz) = tmp(ixx+1:ixx+nx,iyy+1:iyy+ny,izz+1:izz+nz)

    open(unit=8,status='old',file='Fvy.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    fvy(1:nx,1:ny,1:nz) = tmp(ixx+1:ixx+nx,iyy+1:iyy+ny,izz+1:izz+nz)

    open(unit=8,status='old',file='Fvz.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    fvz(1:nx,1:ny,1:nz) = tmp(ixx+1:ixx+nx,iyy+1:iyy+ny,izz+1:izz+nz)

    open(unit=8,status='old',file='den.dat',form='unformatted',access='direct',recl=nnnn)
    read(8,rec=1) tmp
    close(8)
    np(1:nx*pex,1:ny*pey,1:nz*pez)=tmp-n_0
    np(0,:,:)=np(nx*pex,:,:) ; np(nx*pex+1,:,:)=np(1,:,:)
    np(:,0,:)=np(:,ny*pey,:) ; np(:,ny*pey+1,:)=np(:,1,:)
    np(:,:,0)=np(:,:,nz*pez) ; np(:,:,nz*pez+1)=np(:,:,1)

   print*,'Fvx minmax',minval(fvx(:,:,1)), maxval(fvx(:,:,1))
   print*,'Fvy minmax',minval(fvy(:,:,1)), maxval(fvy(:,:,1))
   print*,'Fvz minmax',minval(fvz(:,:,1)), maxval(fvz(:,:,1))

!for the boundary to be called correctly, make sure fvx and fvy has three dependences (x,y,z)
...
! set electric and magnetic field (no kinetic equilibrium with guide field bz)
  do z=1,nz; do y=1,ny; do x=1,nx
    b1x(x,y,z)=fbx(x,y,z); b1y(x,y,z)=fby(x,y,z); b1z(x,y,z)=fbz(x,y,z)
  enddo; enddo; enddo
...
! Find the nearest grid points for the particle.
    ...
    zz=int(rvi(3,np_i)/dz+0.5)
    xm=rvi(1,np_i)/dx+0.5-xx; xp=1.-xm
    ym=rvi(2,np_i)/dy+0.5-yy; yp=1.-ym
    zm=rvi(3,np_i)/dz+0.5-zz; zp=1.-zm
! Interpolate the density perturbations to the particle position.
    npert = (np(xx  ,yy  ,zz)*xp*yp*zp + np(xx  ,yy  ,zz+1)*xp*yp*zm + &
             np(xx+1,yy  ,zz)*xm*yp*zp + np(xx+1,yy  ,zz+1)*xm*yp*zm + &
             np(xx  ,yy+1,zz)*xp*ym*zp + np(xx  ,yy+1,zz+1)*xp*ym*zm + &
             np(xx+1,yy+1,zz)*xm*ym*zp + np(xx+1,yy+1,zz+1)*xm*ym*zm ) 
...
!   write(6,*) 'xm,ym,zm,xp,yp,zp',xm,ym,zm,xp,yp,zp
   ...
   if (pez*nz == 1) then
    ...
    vix = (fvx(xx  ,yy  ,zz)*xp*yp*zp + fvx(xx  ,yy  ,zz+1)*xp*yp*zm + &
           fvx(xx+1,yy  ,zz)*xm*yp*zp + fvx(xx+1,yy  ,zz+1)*xm*yp*zm + &
           fvx(xx  ,yy+1,zz)*xp*ym*zp + fvx(xx  ,yy+1,zz+1)*xp*ym*zm + &
           fvx(xx+1,yy+1,zz)*xm*ym*zp + fvx(xx+1,yy+1,zz+1)*xm*ym*zm )
    viy = (fvy(xx  ,yy  ,zz)*xp*yp*zp + fvy(xx  ,yy  ,zz+1)*xp*yp*zm + &
           fvy(xx+1,yy  ,zz)*xm*yp*zp + fvy(xx+1,yy  ,zz+1)*xm*yp*zm + &
           fvy(xx  ,yy+1,zz)*xp*ym*zp + fvy(xx  ,yy+1,zz+1)*xp*ym*zm + &
           fvy(xx+1,yy+1,zz)*xm*ym*zp + fvy(xx+1,yy+1,zz+1)*xm*ym*zm )
    viz = (fvz(xx  ,yy  ,zz)*xp*yp*zp + fvz(xx  ,yy  ,zz+1)*xp*yp*zm + &
           fvz(xx+1,yy  ,zz)*xm*yp*zp + fvz(xx+1,yy  ,zz+1)*xm*yp*zm + &
           fvz(xx  ,yy+1,zz)*xp*ym*zp + fvz(xx  ,yy+1,zz+1)*xp*ym*zm + &
           fvz(xx+1,yy+1,zz)*xm*ym*zp + fvz(xx+1,yy+1,zz+1)*xm*ym*zm )
   endif

   print*,'vix,viy,viz',vix,viy,viz,fbx(xx,yy,1),xm,ym
