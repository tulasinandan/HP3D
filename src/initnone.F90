...
!! THE FOLLOWING LOOKS LIKE SPURIOUS CODE AND HENCE COMMENTED OUT. WE
!! DO NOT NEED ANY SIN FUNCTIONS IN INITNONE
!!                               TULASI - 2015.06.25
...
!     call veloadd(rvi(4,np_i),rvi(5,np_i),rvi(6,np_i), &
!        real(-sin(2*pi2*rvi(2,np_i)/ly),drk), real(sin(2*pi2*rvi(1,np_i)/lx),drk), real(0.,drk) )
!     call veloadd(rvi(4,np_i),rvi(5,np_i),rvi(6,np_i), &
!        real(-sin(3*pi2*rvi(2,np_i)/ly),drk), real(sin(3*pi2*rvi(1,np_i)/lx),drk), real(0.,drk) )
