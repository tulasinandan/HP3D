...
!  +4*cos(2*pi2*xx/lx)*cos(pi2*yy/ly)/5+cos(2*pi2*yy/ly)/2)*bpert**2
...
! set magnetic field
  do z=1,nz; do y=1,ny; do x=1,nx
    b1x(x,y,z)=-sin(pi2*gy(y)/ly)*bpert; b1y(x,y,z)=sin(2*pi2*gx(x)/lx)*bpert; b1z(x,y,z)=b0z
  enddo; enddo; enddo 
!-------------------------------------------
...
! load ions 
 do i=1,nx*ny*nz*ppg*nsup/n_0
    ...
    call veloadd(rvi(4,np_i),rvi(5,np_i),rvi(6,np_i), &
        real(-sin(pi2*rvi(2,np_i)/ly),drk)*bpert, real(sin(pi2*rvi(1,np_i)/lx),drk)*bpert, real(0.,drk) )
 enddo
...
#endif
  enddo; enddo; enddo 
    write(*,*) 'Done initializing, writing the startup file'
! write start-up file
  call output(0,.true.)
End Subroutine init_OTV_hyb
