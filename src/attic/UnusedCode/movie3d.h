...
! open movie files
  if (myproc == 0) then
    ...
    open(unit=11,file='rho',action='write',form='unformatted',access='direct',recl=mrecl)
    ...
    open(unit=21,file='ni', action='write',form='unformatted',access='direct',recl=mrecl)
...
! write to movie files
  if (t >= t_movieout) then
    ...
    call volume(rho,11,10)
    call volume(jx,12,10)
    call volume(jy,13,10)
    call volume(jz,14,10)
    call volume(b1x,15,10) 
    call volume(b1y,16,10)
    call volume(b1z,17,10)
    call smooth_e(e1x,e1y,e1z,esx,esy,esz)
    call volume(esx,18,10)
    call volume(esy,19,10)
    call volume(esz,20,10)
    call calc_rho(rvi,np_i,real(+1.,drk),.true.)
    call volume(rho,21,10)
    call calc_rho(rve,np_e,real(+1.,drk),.true.)
    call volume(rho,22,10)
...
! close movie files
  close(10); close(11); close(12); close(13); close(14); close(15); close(16)
     close(17); close(18); close(19); close(20); close(21); close(22)
#endif
