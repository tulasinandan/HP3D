!******************************************************************************
!             3D particle code: Standing Alfven wave setup.
!                           template by Andreas Zeiler, 1999
!                           Modified for wave by Michael Shay, 2004
!                           Further modified by Michael Shay, 2006.
!
...
!
!############## Beginning of Alfven wave initialization file
!
! M. Shay: 6/25/2006:        Setup up standing Alfven wave. 
!
!******************************************************************************
...
! function which is not executed until after c_si2 is defined. 
  ...
  ni(xx)     = 1.0
  ne(xx)     = 1.0
  Bperty(xx) = bpert*sin(kval*xx)
  vix(xx)    = 0. + (v_waveadd)
  viy(xx)    = 0.0
  Ezval(xx)   = 0.0
  vex(xx)    = vix(xx)
  vey(xx)    = viy(xx)
  vez(xx)    = -bpert*kval*cos(kval*xx)
...
! set electric and magnetic field (no kinetic equilibrium with guide field bz)
  do z=1,nz; do y=1,ny; do x=1,nx
    b1x(x,y,z)=1.0; b1y(x,y,z)=Bperty(gx(x)); b1z(x,y,z)=0.
    e1x(x,y,z)=0.; e1y(x,y,z)=0.; e1z(x,y,z)=Ezval(gx(x))
  enddo; enddo; enddo 
...
do z=1,nz ; do y=1,ny ; do x=1,nx
  ...
  do i=1,numpartgrid
     ...
     call veloadd(rvi(4,np_i),rvi(5,np_i),rvi(6,np_i), &
        vix(real(rvi(1,np_i),drk)),viy(real(rvi(1,np_i),drk)),real(0.,drk) )
  end do
...
do z=1,nz ; do y=1,ny ; do x=1,nx
  ...
  do i=1,numpartgrid
     ...
     call veloadd(rve(4,np_e),rve(5,np_e),rve(6,np_e), &
       vex(real(rve(1,np_e),drk)),vey(real(rve(1,np_e),drk)),vez(real(rve(1,np_e),drk)) )
  end do
end do ; end do ; end do
