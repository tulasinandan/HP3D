...
! Removed a +100*lx,ly,lz from the position step and added ifs around
! the mod step.  The former was to make sure position was never
! negative; that is now done in the mod step.  Note that the F90 version
...
! mods is used instead. Quasi-2D means a total of 2 grid points
...
! (called by stepx)
  ...
  Integer, Parameter :: maxbuff = 10000
  ...
  Real(kind=drk) :: overflow

  Integer :: procval
  Logical :: slow_ex, again
...
! between each possible pair of processors.  For large N that is painful.
...
! MS: Calculate ipdist here --- the number of particles that belong on
! each processor.  The important number is which particles belong on a
! processor other than the current one.  Calculating it here
! eliminates a loop over np within the main do loop.
...
!   send buffer in this turn and the again-flag is set.
    ...
    again = .false.     ! Flag if we have to loop again
...
!   New variable
	...
	! MS: Update ipdist.  Note that we do not care if the ipdist
	! count of the current processor is wrong; instead only the
	! count for other processors matters.
        ipdist(ip) = ipdist(ip)-1
      endif
    enddo
...
! NEW.  If the slow exchange model is necessary, step over every processor.
    ...
    do i=1,procval
! NEW.  Step over processors correctly.  For slow case, step over
...
! Exchange the number of particles we will send in this turn
      call MPI_Sendrecv (ipsend(iprocs),1,MPI_INTEGER,iprocs,i, &
        npr,1,MPI_INTEGER,iprocr,i,MPI_COMM_WORLD, &
        mpi_status,mpi_err)
...
! 7*ipsend(iprocs) elements.  
      call MPI_Sendrecv ( &
        sendbuff(0,ipstart(iprocs)+1),7*ipsend(iprocs),mpi_prk,iprocs,i, &
        recvbuff,7*npr,mpi_prk,iprocr,i,MPI_COMM_WORLD, &
        mpi_status,mpi_err)

!   Check for particle buffer overflow, exit if that happens
      if (np+npr > maxparticles) then
        write(6,*) &
          '***** redistribute: particle buffer overflow, abort *****', np, npr, myproc,i,slow_ex,ipsend(iprocs)
...
!     Store the particles received in the particles buffer
    ...
    again = reducesum_logical(again)
    call MPI_Bcast(again,1,mpi_logical,0,MPI_COMM_WORLD,mpi_err)
  enddo
