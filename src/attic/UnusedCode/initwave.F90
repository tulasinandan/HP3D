...
! 1D equilibrium
! Don't worry. c_s is defined after vix(xx), but vix(xx) is a little
! function which is not executed until after c_s is defined. 

  ntwid(xx) = npert*sin(kval*xx)
  ni(xx) = n_0 + ntwid(xx)
  ne(xx) = n_0 + ntwid(xx)/debcorr
  vix(xx) = c_s*ntwid(xx)/(n_0*sqrt(debcorr))
  vex(xx) = c_s*ntwid(xx)/(n_0*(debcorr)**1.5)
  Eval(xx) = -(1.*npert/n_0)*cos(kval*xx)*kval*c_s**2/debcorr

! Some variables for the equilibrium

  edebye = sqrt(1.*T_e/c_2)
  kval = 6.28318530718/lx
  debcorr = 1. + (edebye*kval)**2
