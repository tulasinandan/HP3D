...
! write distribution
  write(filename,"('vdixy.',i4.4)") count
  open(unit=20,file=filename,action='write')
  write(filename,"('vdiyz.',i4.4)") count
  open(unit=21,file=filename,action='write')
  write(filename,"('vdixz.',i4.4)") count
...
! write distribution
  write(filename,"('vdexy.',i4.4)") count
  open(unit=20,file=filename,action='write')
  write(filename,"('vdeyz.',i4.4)") count
  open(unit=21,file=filename,action='write')
  write(filename,"('vdexz.',i4.4)") count
  open(unit=22,file=filename,action='write')
  do j=-vbin,vbin; do i=-vbin,vbin
    write(20,*) vxy(i,j); write(21,*) vyz(i,j); write(22,*) vxz(i,j)
  enddo; enddo
...
#ifdef USE_SIMPLE_IO
  Real(kind=drk), Dimension(1:nx,1:ny,1:nz):: tmp
#else
