...
#define moviecombine_variables
#include movie_header
#undef moviecombine_variables
Character, Dimension(nx,ny,nz,nvalues) :: chardata
Character, Dimension(nx*pex,ny*pey,nz*pez,nvalues) :: charfull

#define movie_dataout_array
#include movie_header
#undef movie_dataout_array
...
do ival=1,nvalues
  open(unit=10+ival,file=valoutput(ival),status='unknown',form='unformatted' &
      ,access='direct',recl=nx*pex*ny*pey*nz*pez)
end do

! Open raw movie files to read in
do iproc=0,nprocs-1
! write(filename,"(a,'.',i4.4,'.',i1.1)") 'movie',iproc,stepnum
  write(filename,"(a,'.',i6.6)") 'movie',iproc
  open (unit=100+iproc,file=filename,status='old',action='read',form='unformatted' &
       ,access='direct',recl=nx*ny*nz*nvalues)
end do
