...
export CODEDIR=$HOME/CODES/HP3D
export SCRATCH=/nfs/scratch/parashtu/HP3D
export PAR=OTV2D  # paramfile = param_$PAR
export NSTEPS=10
...
###########  Begin Run  #####################################
 ...
 echo $0
    ...
    if [ "$RESTART" = "0" ]; then 
    mpirun -n $NPROCS $DR/init-hybrid >& init.stdout
     echo "####################### CREATED THE INITIAL FILES ######################"
    fi
    mpirun -n $NPROCS $DR/p3d-hybrid $FILENO $NSTEPS >& p3d.stdout
